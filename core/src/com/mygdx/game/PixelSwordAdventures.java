...
public class PixelSwordAdventures extends ScreenAdapter {
  ...
  Music menu, easteregg;
  boolean esateregg = false;
  ...
  public PixelSwordAdventures(Game game) {
    ...
    //Music
    this.menu =
      Gdx.audio.newMusic(Gdx.files.internal("ressources/Music/Menu.mp3"));
    this.menu.setLooping(true);
    this.menu.setVolume(0.5f);

    this.easteregg =
      Gdx.audio.newMusic(Gdx.files.internal("ressources/Music/Menu2.mp3"));
    this.easteregg.setLooping(true);
    this.easteregg.setVolume(0.5f);
  ...
  public void render(float delta) {
    ...
    if (GameTable.showMenu) {
      ...
      if (Gdx.input.justTouched()) {
        ...
        // i want detect if the hero is on the tile with the red mushroom
        if (
                enemies.isEmpty() &&
                        hero != null &&
        ...
        if (newLevel && !boss) {
          ...
          for (int i = 0; i < nbEnemies; i++) {
            ...
            int attack = 20 + (i *20);
            int hp = 100 + (i * 50);
            Enemy enemy = new Enemy(enemyName, hp, attack, 10, null, "area1", 1);
            gobelin.test(enemy);
            enemies.put(enemy, gobelin);
            gobelin.reset(box2D, island.getRandomTile());
          }
        ...
        if (hero != null) {
          ...
          font.draw(ui, "Attack: " + player.getAttack(), 10, 200);
          font.draw(ui, "Defense: " + player.getDefense(), 10, 170);
